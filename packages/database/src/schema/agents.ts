...
export const agents = pgTable(
  ...
  (table) => ({
    ...
    downloadsCheck: check("downloads_check", sql`${table.downloadsCount} >= 0`),
    forksCheck: check("forks_check", sql`${table.forksCount} >= 0`),
    starsCheck: check("stars_check", sql`${table.starsCount} >= 0`),
